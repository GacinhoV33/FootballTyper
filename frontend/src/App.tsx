...
function App() {
  ...
  const maxBetsNbr = allMatches
    ? allMatches.filter((match) => match.awayTeam !== null).length
    : 64;
  ...
  const groupStageReturn = () => {
    if (!userStatus.isUserSigned) {
      ...
      return <LoadingLayout componentName="Group Stage" />;
    }
  };
      ...
      <div className="app-body">
        ...
        <Routes>
          <Route
            ...
            element={
              allMatches && allTeams ? (
                <Homepage allTeams={allTeams} allMatches={allMatches} />
              ) : (
                <LoadingLayout componentName="Homepage" />
              )
            }
          />
          <Route
            path="/knockout"
            element={
              userStatus.isUserSigned ? (
                <KnockoutStage allMatches={allMatches} />
              ) : (
          ...
          <Route
            ...
            element={
              allUserBets !== null ? (
                ...
                <LoadingLayout componentName="My bets" />
              )
            }
          />
...
function convertMatchesToGroupFormat(data: any) {
  ...
  let tab: any = [];
  for (let i = 0; i < 8; i++) {
    const result = data.filter((match: any) =>
      match.group ? match.group[6] === letters[i] : false
    );
    tab.push(result);
  }
  return tab;
