...
const MatchCard: React.FC<MatchCardProps> = ({
  ...
  const isBetNew =
    userBets !== null && userBets.length > 0
      ? userBets.filter((bet) => bet.matchId === match.id)
      : null;
  const isBetExisting =
    isBetNew !== null &&
    isBetNew.length !== 0 &&
  ...
  return (
    ...
    >
      ...
      <div className="match-card">
        <div className="team left-team">{homeTeam}</div>

        <div className="left-flag">
          <CircleFlag
            className="flag-matchcard"
            countryCode={CountryDict.get(homeTeam) as string}
          />
        </div>
        ...
        <div className="vs-pill">
          ...
          >
            <GiDervishSwords
              style={{ height: "4vh", minHeight: "40px", minWidth: "30px" }}
            />
          </div>
        </div>
        ...
        <div className="right-flag">
          <CircleFlag
            className="flag-matchcard"
            countryCode={CountryDict.get(awayTeam) as string}
          />
        </div>

        <div className="team right-team">{awayTeam}</div>
      </div>
      {showBet && (
        <BetModal
          showBet={showBet}
