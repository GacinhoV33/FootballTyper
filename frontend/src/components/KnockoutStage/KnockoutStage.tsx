...
const CustomSeed = ({
  ...
  const isBetAllowed =
    !seed.isMatchPlayed &&
    seed.teams[0].name !== "" &&
    seed.teams[1].name !== ""; //TODO set time
  const [userBets, setUserBets] = useState<Bet[] | null>(seed.userBets);
  const isBetNew =
    userBets !== null && userBets.length > 0
      ? userBets.filter((bet: Bet) => bet.matchId === seed.groupMatch.id)
      : null;
  const [isBetExisting, setIsBetExisting] = useState<boolean>(
    isBetNew !== null &&
      isBetNew.length !== 0 &&
      isBetNew[0].homeTeamScoreBet !== undefined
  );
    ...
    <Seed mobileBreakpoint={breakpoint} style={{ fontSize: 14, width: "20vw" }}>
      ...
      >
          ...
          <SeedTeam style={wonTeam === 1 ? styleWonTeam : undefined}>
            ...
            >
              <div>
                <CircleFlag
                  countryCode={
                    CountryDict.get(seed.teams[0].name as string) as string
                  }
                  height="30"
                ...
                <span
                  className={
                    wonTeam === 1 ? "won-team-score team-text" : "team-text"
                  }
                >
                  {seed.groupMatch.homeTeamScore !== -1
                    ? seed.groupMatch.homeTeamScore
                    : null}
                </span>
              </div>
              <div
              ...
              >
                {betChange !== 0
                  ? modalValue.homeScore
                  : isBetExisting
                  ? `(${
                      isBetNew !== null ? isBetNew[0].homeTeamScoreBet : null
                    })`
                  : null}
              </div>
            </div>
          ...
          <SeedTeam style={wonTeam === 0 ? styleWonTeam : undefined}>
            ...
            >
              <div>
                <CircleFlag
                  countryCode={
                    CountryDict.get(seed.teams[1].name as string) as string
                  }
                  height="30"
                  style={{ marginRight: "0.5vw" }}
                />
                ...
                <span
                  className={
                    wonTeam === 0 ? "won-team-score team-text" : "team-text"
                  }
                >
                  {seed.groupMatch.awayTeamScore !== -1
                    ? seed.groupMatch.awayTeamScore
                    : null}
                </span>
              </div>
              <div
              ...
              >
                {betChange !== 0
                  ? modalValue.awayScore
                  : isBetExisting && isBetNew !== null
                  ? `(${isBetNew[0].awayTeamScoreBet})`
                  : null}
              </div>
            </div>
          </SeedTeam>
        ...
        <div style={{ paddingRight: "1vw" }}>
          {!seed.groupMatch.isMatchValid &&
          new Date(seed.groupMatch.date) > new Date() ? (
            <Button
            ...
            isBetNew[0].betResult === 1 ? (
              <BsCheck size={40} style={{ color: "lightgreen" }} />
            ) : isBetNew[0].betResult === 2 ? (
              <BiCheckDouble size={40} style={{ color: "darkgreen" }} />
      ...
      {showAlert ? (
        Number(modalValue.homeScore) < 100 &&
        Number(modalValue.homeScore) >= 0 &&
        Number(modalValue.awayScore) >= 0 &&
        Number(modalValue.awayScore) < 100 ? (
          <AlertAnimation>
...
const MobilePhoneKnockout: React.FC<MobilePhoneKnockoutProps> = ({
  rounds,
}) => {
  const userCtx = useContext(UserContext);
  const API_URL =
...
const KnockoutStage: React.FC<KnockoutStageProps> = ({ allMatches }) => {
  ...
  function createData() {
    ...
    const oneEightMatches = allMatches
      ?.filter((match) => match.stage === 1)
      .sort(
        (a, b) =>
          oneEightOrderDict[a.matchNumber] - oneEightOrderDict[b.matchNumber]
      );
    const seedsOneEight =
      oneEightMatches !== null && oneEightMatches !== undefined
        ? oneEightMatches.map((match) => {
            return {
              id: match.id,
              date: match.date,
              teams: [
    ...
    const quarterMatches = allMatches
      ...
      .sort(
        (a, b) =>
          quarterOrderDict[a.matchNumber] - quarterOrderDict[b.matchNumber]
      );
    const seedsQuarter =
      quarterMatches !== null && quarterMatches !== undefined
    ...
    const semiMatches = allMatches
      ?.filter((match) => match.stage === 3)
      .sort((a, b) => a.matchNumber - b.matchNumber);
    const seedsSemi =
      semiMatches !== null && semiMatches !== undefined
